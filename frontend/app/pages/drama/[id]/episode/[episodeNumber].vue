                ...
                <div v-for="c in chars" :key="c.id" class="extract-row">
                  ...
                  <div class="extract-info">
                    <div class="extract-name-row">
                      ...
                      <select class="gender-select" :value="c.gender || ''" @change="characterAPI.update(c.id, { gender: $event.target.value }), c.gender = $event.target.value">
            ...
            <div class="shot-list">
              ...
              <div class="shot-list-body">
                ...
                >
                  ...
                  <div class="shot-body">
                    <div v-if="getStoryboardCover(sb)" class="shot-thumb" @click.stop="openImageViewer('/' + getStoryboardCover(sb), `镜头 #${i+1} 图片`)">
                      <img :src="'/' + getStoryboardCover(sb)" />
                      <div class="shot-thumb-overlay" @click.stop="uploadSbFrame(sb, 'composedImage')">
                        <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
                        替换
                      </div>
                    </div>
                ...
                <div class="detail-hero">
                  ...
                  <div class="detail-preview-grid">
                    <div class="detail-preview-card">
                      ...
                      <div class="detail-preview-media" @click="uploadSbFrame(selectedSb, 'firstFrameImage')">
                        <img
                        ...
                        <div v-if="getFirstFrame(selectedSb)" class="preview-overlay" @click.stop="uploadSbFrame(selectedSb, 'firstFrameImage')">
                    ...
                    <div class="detail-preview-card">
                      ...
                      <div class="detail-preview-media" @click="uploadSbFrame(selectedSb, 'lastFrameImage')">
                        <img
                        ...
                        <div v-if="getLastFrame(selectedSb)" class="preview-overlay" @click.stop="uploadSbFrame(selectedSb, 'lastFrameImage')">
                          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
                          替换
                        </div>
                      </div>
          ...
          <div v-if="prodTab === 'chars'" class="prod-content">
            ...
            <div class="asset-grid">
              <div v-for="c in visualChars" :key="c.id" class="card asset-card">
                <div class="asset-cover" @click="uploadCharImg(c)">
                  <img
                  ...
                  <div v-if="c.image_url || c.imageUrl" class="asset-cover-overlay" @click.stop="uploadCharImg(c)">
          ...
          <div v-else-if="prodTab === 'scenes'" class="prod-content">
            ...
            <div class="asset-grid">
              <div v-for="s in scenes" :key="s.id" class="card asset-card">
                <div class="asset-cover wide" @click="uploadSceneImg(s)">
                  <img
                  ...
                  <div v-if="s.image_url || s.imageUrl" class="asset-cover-overlay" @click.stop="uploadSceneImg(s)">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
                    替换
                  </div>
                </div>
                  ...
                  <div class="frame-thumbs">
                    <div class="frame-thumb-wrap">
                      <div class="frame-thumb" @click.stop="!isPendingShotFrame(sb.id, 'first_frame') && genShotFrame(sb, 'first_frame')">
                        ...
                        <span v-if="getFirstFrame(sb)" class="frame-re" @click.stop="uploadSbFrame(sb, 'firstFrameImage')">
                          <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
                        </span>
                      </div>
                    ...
                    <div v-if="frameMode === 'first_last'" class="frame-thumb-wrap">
                      <div class="frame-thumb" @click.stop="!isPendingShotFrame(sb.id, 'last_frame') && genShotFrame(sb, 'last_frame')">
                        ...
                        <span v-if="getLastFrame(sb)" class="frame-re" @click.stop="uploadSbFrame(sb, 'lastFrameImage')">
                          <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
                        </span>
                      </div>
            ...
            <div class="prod-grid">
              <div v-for="(sb, i) in sbs" :key="sb.id" class="card prod-card">
                <div class="prod-cover">
                  <video
                    v-if="hasVid(sb) && !videoLoadErrors[sb.id]"
                    :src="'/' + getVideoUrl(sb) + '?t=' + videoCacheKey"
                    class="prod-video"
                    controls
                    preload="metadata"
                    playsinline
                    @error="handleVideoError(sb.id)"
                  />
                  <img
                    v-else-if="hasImg(sb)"
          ...
          <div v-else-if="prodTab === 'compose'" class="prod-content">
            ...
            <div class="prod-grid">
              <div v-for="(sb, i) in sbs" :key="sb.id" class="card prod-card">
                <div class="prod-cover">
                  <video
                    v-if="hasComposed(sb) && !videoLoadErrors[sb.id]"
                    :src="'/' + getComposedVideoUrl(sb) + '?t=' + videoCacheKey"
                    class="prod-video"
                    controls
                    preload="metadata"
                    playsinline
                  ...
                  />
                  <video
                    v-else-if="hasVid(sb) && !videoLoadErrors[sb.id]"
                    :src="'/' + getVideoUrl(sb) + '?t=' + videoCacheKey"
                    class="prod-video"
                    controls
                    preload="metadata"
...
import { dramaAPI, episodeAPI, storyboardAPI, characterAPI, sceneAPI, imageAPI, videoAPI, composeAPI, mergeAPI, gridAPI, aiConfigAPI, voicesAPI, uploadImage } from '~/composables/useApi'
...
const edgeFallbackVoiceProfiles = [
  { id: 'zh-CN-XiaoxiaoNeural', label: '晓晓', gender: '女声', traits: '温柔、自然、甜美', suitable: '女主、旁白、温柔女性' },
  { id: 'zh-CN-XiaoyiNeural', label: '晓依', gender: '女声', traits: '活泼、可爱、年轻', suitable: '少女、活泼角色' },
  { id: 'zh-CN-YunxiNeural', label: '云希', gender: '男声', traits: '阳光、年轻、活力', suitable: '男主、年轻男性' },
  { id: 'zh-CN-YunjianNeural', label: '云健', gender: '男声', traits: '沉稳、有力、成熟', suitable: '成熟男性、旁白' },
  { id: 'zh-CN-YunyangNeural', label: '云扬', gender: '男声', traits: '专业、播音、稳重', suitable: '新闻播报、旁白、专业场合' },
  { id: 'zh-CN-YunxiaNeural', label: '云夏', gender: '男声', traits: '少年、清朗、朝气', suitable: '少年角色、学生' },
  { id: 'zh-CN-liaoning-XiaobeiNeural', label: '晓北', gender: '女声', traits: '东北话、豪爽、接地气', suitable: '东北方言角色' },
  { id: 'zh-CN-shaanxi-XiaoniNeural', label: '晓妮', gender: '女声', traits: '陕西话、质朴、亲切', suitable: '陕西方言角色' },
  { id: 'zh-HK-HiuGaaiNeural', label: '曉佳', gender: '女声', traits: '粤语、温柔', suitable: '粤语女声' },
  { id: 'zh-HK-HiuMaanNeural', label: '曉曼', gender: '女声', traits: '粤语、优雅', suitable: '粤语优雅女声' },
  { id: 'zh-HK-WanLungNeural', label: '雲龍', gender: '男声', traits: '粤语、沉稳', suitable: '粤语男声' },
  { id: 'zh-TW-HsiaoChenNeural', label: '曉臻', gender: '女声', traits: '台湾腔、温柔', suitable: '台湾女声' },
  { id: 'zh-TW-YunJheNeural', label: '雲哲', gender: '男声', traits: '台湾腔、温暖', suitable: '台湾男声' },
  { id: 'zh-TW-HsiaoYuNeural', label: '曉雨', gender: '女声', traits: '台湾腔、活泼', suitable: '台湾少女' },
]
...
function handleVideoError(sbId) {
  videoLoadErrors.value = { ...videoLoadErrors.value, [sbId]: true }
}
...
const lockedAudioProvider = computed(() => {
  const fromEpisode = audioConfigs.value.find(c => c.id === lockedAudioConfigId.value)?.provider
  if (fromEpisode) return fromEpisode
  const active = audioConfigs.value.find(c => c.is_active || c.isActive)
...
function updateCharVoice(charId, voiceId) {
  // 根据音色 ID 自动推断 provider
  let voiceProvider = lockedAudioProvider.value
  if (voiceId && voiceId.match(/^zh-/)) {
    voiceProvider = 'edge-tts'
  } else if (voiceId && voiceId.includes('voice-')) {
    voiceProvider = 'minimax'
  } else if (voiceId && ['alloy', 'echo', 'fable', 'onyx', 'nova', 'shimmer'].includes(voiceId)) {
    voiceProvider = 'openai'
  }
  characterAPI.update(charId, { voice_style: voiceId, voice_provider: voiceProvider || undefined })
  const c = chars.value.find(ch => ch.id === charId)
  if (c) {
    c.voice_style = voiceId
    c.voiceStyle = voiceId
    c.voice_provider = voiceProvider || ''
    c.voiceProvider = voiceProvider || ''
    c.voice_sample_url = ''
    c.voiceSampleUrl = ''
  }
}
...
async function uploadCharImg(char) {
  ...
  input.onchange = async () => {
    ...
    if (!file) return
    try {
      const path = await uploadImage(file, 'images')
      await characterAPI.update(char.id, { imageUrl: path })
      char.image_url = path
      char.imageUrl = path
...
async function uploadSceneImg(scene) {
  const input = document.createElement('input')
  input.type = 'file'
  input.accept = 'image/*'
  input.onchange = async () => {
    ...
    try {
      const path = await uploadImage(file, 'images')
      await sceneAPI.update(scene.id, { imageUrl: path })
...
async function uploadSbFrame(sb, field) {
  const input = document.createElement('input')
  input.type = 'file'
  input.accept = 'image/*'
  input.onchange = async () => {
    ...
    try {
      const path = await uploadImage(file, 'images')
      await storyboardAPI.update(sb.id, { [field]: path })
      sb[field] = path
      toast.success('镜头图片已上传')
    } catch (e) {
      toast.error(e.message)
    }
  }
...
const videoCacheKey = ref(Date.now())
const prevVideoUrls = ref('')
function updateVideoCacheKey() {
  const urls = sbs.value.map(s => [s.video_url || s.videoUrl, s.composed_video_url || s.composedVideoUrl].join('|')).join(',')
  if (urls !== prevVideoUrls.value) {
    prevVideoUrls.value = urls
    videoCacheKey.value = Date.now()
  }
}
...
function inferVoiceGender(name, desc = []) {
  ...
  if (/female|woman|girl|女|少女|御姐|奶奶/i.test(text)) return '女声'
  if (/\bmale|man|boy|男|青年|大爷|学长/i.test(text)) return '男声'
  return '中性'
}

const EDGE_VOICE_LABELS = {
...
function mapVoiceProfile(v) {
  ...
  const gender = inferVoiceGender(v.voice_name || v.voice_id, desc)
  const genderLabel = gender === '女声' ? '女声' : gender === '男声' ? '男声' : '中性'

  if (v.provider === 'edge-tts') {
    const label = EDGE_VOICE_LABELS[v.voice_id] || v.voice_id
    const locale = desc.find(d => d.startsWith('zh-')) || ''
    const region = EDGE_LOCALE_LABELS[locale] || locale
    return {
      id: v.voice_id,
      label,
      gender,
      traits: `${genderLabel}·${region}`,
      suitable: `${region}角色`,
    }
  }

  const localDesc = desc.map(d => d === 'Female' ? '女声' : d === 'Male' ? '男声' : d)
  return {
    id: v.voice_id,
    label: v.voice_name || v.voice_id,
    gender,
    traits: localDesc.length ? localDesc.slice(0, 2).join('、') : `${v.language || '多语言'}音色`,
    suitable: `${v.language || '通用'}角色`,
  }
}

async function loadVoices() {
  try {
    const provider = lockedAudioProvider.value || 'openai'
    const rows = await voicesAPI.list(provider)
    voiceProfiles.value = rows?.length ? rows.map(mapVoiceProfile) : (provider === 'edge-tts' ? edgeFallbackVoiceProfiles : fallbackVoiceProfiles)
  } catch (e) {
    console.error('Failed to load voices', e)
    voiceProfiles.value = lockedAudioProvider.value === 'edge-tts' ? edgeFallbackVoiceProfiles : fallbackVoiceProfiles
  }
}
...
.shot-thumb-overlay {
  ...
  background: rgba(0,0,0,.55); color: #fff; font-size: 10px; cursor: pointer;
  opacity: 0; transition: opacity .15s;
}
.shot-thumb:hover .shot-thumb-overlay { opacity: 1; }
...
.asset-cover { position: relative; aspect-ratio: 1; background: var(--bg-2); overflow: hidden; cursor: pointer; }
...
.asset-cover-empty { width: 100%; height: 100%; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 4px; color: var(--text-3); cursor: pointer; }
.upload-hint { font-size: 10px; color: var(--text-3); }
.asset-cover-overlay {
  position: absolute; inset: 0; display: flex; align-items: center; justify-content: center; gap: 4px;
  background: rgba(0,0,0,.55); color: #fff; font-size: 11px; cursor: pointer; opacity: 0; transition: opacity .15s;
}
.asset-cover:hover .asset-cover-overlay { opacity: 1; }
.preview-overlay {
  position: absolute; inset: 0; display: flex; align-items: center; justify-content: center; gap: 4px;
  background: rgba(0,0,0,.55); color: #fff; font-size: 10px; cursor: pointer; opacity: 0; transition: opacity .15s;
}
.detail-preview-media:hover .preview-overlay { opacity: 1; }
