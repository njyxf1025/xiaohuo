...
async function req<T = any>(method: string, path: string, body?: any): Promise<T> {
  ...
  devLog(`%c[API] %c${method} %c${path}`, 'color:#888', 'color:#4fc3f7;font-weight:bold', 'color:#ccc', body || '')
  ...
  try {
    ...
    if (!resp.ok || (json.code && json.code >= 400)) {
      devLog(`%c[API] %c${method} ${path} %c${resp.status} %c${ms}ms`, 'color:#888', 'color:#ef5350', 'color:#ef5350;font-weight:bold', 'color:#888', json.message || '')
      throw new Error(json.message || `${resp.status}`)
    }

    devLog(`%c[API] %c${method} ${path} %c${resp.status} %c${ms}ms`, 'color:#888', 'color:#66bb6a', 'color:#66bb6a;font-weight:bold', 'color:#888')
    return json.data ?? json
  } catch (err: any) {
    if (!err.message?.match(/^\d{3}$/)) {
      const ms = Math.round(performance.now() - start)
      devLog(`%c[API] %c${method} ${path} %cERROR %c${ms}ms`, 'color:#888', 'color:#ef5350', 'color:#ef5350;font-weight:bold', 'color:#888', err.message)
    }
    throw err
  }
}
...
export async function uploadImage(file: File, subDir = 'images'): Promise<string> {
  const formData = new FormData()
  formData.append('file', file)
  formData.append('sub_dir', subDir)
  const resp = await fetch(`${BASE}/upload/image`, { method: 'POST', body: formData })
  const json = await resp.json()
  if (!resp.ok) throw new Error(json.message || '上传失败')
  return json.data?.path || json.path
}
