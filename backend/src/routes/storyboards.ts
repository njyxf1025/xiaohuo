...
app.post('/:id/generate-tts', async (c) => {
  ...
  const [ep] = db.select().from(schema.episodes).where(eq(schema.episodes.id, sb.episodeId)).all()
  try {
    const audioPath = await generateTTS({ text: pureDialogue, voice: voiceId, configId: ep?.audioConfigId || null, voiceProvider })
  db.update(schema.storyboards)
    ...
    logTaskError('StoryboardAPI', 'generate-tts', { storyboardId: id, voiceId, voiceProvider, error: msg })
