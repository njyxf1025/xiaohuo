  ...
  for (let i = 0; i < 300; i++) {
    ...
    try {
      ...
      if (pollResp.status === 'completed' && pollResp.videoUrl) {
        ...
        const rec = db.select({ duration: schema.videoGenerations.duration, storyboardId: schema.videoGenerations.storyboardId }).from(schema.videoGenerations).where(eq(schema.videoGenerations.id, id)).get()
        await handleVideoComplete(id, pollResp.videoUrl, rec?.duration, rec?.storyboardId)
        return
      }
